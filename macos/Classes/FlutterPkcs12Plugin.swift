  ...
  private func executeFunc(call: FlutterMethodCall, result:FlutterResult) throws {
    ...
    private func signDataWithP12(_ p12Bytes: NSData, _ password: NSString, _ data: NSData, _ signatureHashType : SignatureHashType, _ result: FlutterResult) throws {
        ...
        guard resultSign.error == nil else {
            result(FlutterError(code: "ERROR_SIGN", message: "It was not possible to sign using the private key \(resultSign.error!.localizedDescription)", details: nil))
            return
        }
    ...
    private func sign(data plainData: Data, privateKey: SecKey!, signatureHashType: SignatureHashType) -> RSASigningResult {
        ...
        guard let signaturex = SecKeyCreateSignature(privateKey,
                                                    algorithm,
                                                    plainData as CFData,
                                                    &error) as Data? else {
            return (signedData: nil, error: error!.takeRetainedValue() as Optional<Error>)
        }

        if err == errSecSuccess {
            return (signedData: Data(_: signaturex), error: nil)
        }
